...
public class UtilsType {
	...
	public static boolean isColoredShulkerBox(Material m) {
		switch(m) {
	...
	public static boolean isShulkerBox(Material m) {
		if(isColoredShulkerBox(m) || m.equals(Material.SHULKER_BOX)) return true;
		return false;
	}
	...
	public static boolean isTransparent(Material m) {
		if (isAir(m) || is_banner(m) || is_bed(m) || isCarpet(m) || isWallBanner(m) || isTrapdoor(m) || isDoor(m) || isGate(m)
	...
	public static boolean isInteractable(Material m) {
		return isShulkerBox(m) || isButton(m) || isWoodenDoor(m) || isGate(m) || isWoodenTrapdoor(m)
	...
	public static boolean isWallBanner(Material m) {
		switch(m) {
	...
	public static boolean isColoredTerracotta(Material m) {
		switch(m) {
	...
	public static boolean isTerracotta(Material m) {
		if(isColoredTerracotta(m) || m.equals(Material.TERRACOTTA)) return true;
		return false;
	}
	...
	public static boolean isPlant(Material m) {
		return isFlower(m) || isMushroom(m) || isSapling(m) || m == Material.DEAD_BUSH || m == Material.VINE || m == Material.LILY_PAD
